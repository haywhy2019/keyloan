    ...
    return (
      ...
      >
        ...
        >
          ...
          <Text style={[GLOBAL_STYLE.h2, { color: COLORS.primaryBlue2 }]}>
            {item?.data_amount?.length >= 4 ? `${item.data_amount/1000}GB `  : `${item.data_amount}MB`} for {item.topup_currency}{item.price}
          </Text>
        </View>
      </Pressable>
    );
          ...
          <View style={{ marginHorizontal: "5%", marginTop: 20 }}>
            ...
            {dataPlan.length > 0 && (
              <Input
                ...
                // onChangeText={handleChange("phoneNumber")}
                // onChangeText={(text) => setPhoneNumber(text)}
                onPress={() => setModalDataPlan(true)}
                onBlur={handleBlur("amount")}
                value= {amount == "" ? "Select Data Plan" : amount?.data_amount?.length >= 4 ? `${amount.data_amount/1000}GB for ${amount.topup_currency} ${amount.price}`  :  `${amount.data_amount}MB for ${amount.topup_currency} ${amount.price}`}
